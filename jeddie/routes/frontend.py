"""
Frontend routes for the wedding website.

This module contains all the frontend routes for the wedding website, including:
- Static pages (home, story, wedding details, photos, hotel)
- RSVP functionality
- Gift registry with Stripe integration
"""
...
def add_language_code(endpoint, values):
    """
    Add language code to all URLs generated by url_for.
    
    Args:
        endpoint: The endpoint being generated
        values: The values being used to generate the URL
    """
...
def pull_lang_code(endpoint, values):
    """
    Extract language code from URL and load appropriate translations.
    ...
    Raises:
        404: If the language code is not supported
    """
...
def story():
    """
    Render the couple's story page.
    
    Returns:
        str: Rendered story page template
    """
...
def wedding():
    """
    Render the wedding details page.
    
    Returns:
        str: Rendered wedding details template
    """
...
def rsvp():
    """
    Handle initial RSVP page display.
    
    Returns:
        str: Rendered RSVP page template
    """
...
def rsvp_search():
    """
    Handle RSVP name search functionality.
    
    Validates reCAPTCHA and searches for matching guests. Redirects to appropriate
    page based on number of matches found.
    
    Returns:
        Response: Redirect to RSVP detail page or RSVP selection page
        
    Raises:
        InvalidRSVPException: If reCAPTCHA validation fails or no matches found
    """
...
def rsvp_detail(party_id: str):
    """
    Handle party-level RSVP details.
    
    Args:
        party_id: UUID of the party
        
    Returns:
        Response: Rendered RSVP detail template for the next unfinalized guest,
            or redirect to the thank you page when all guests have finalized
            their RSVPs.
        
    Raises:
        InvalidRSVPException: If party not found
    """
    session = get_db()
...
def rsvp_detail_guest(party_id: str, guest_id: int):
    """
    Handle individual guest RSVP details.
    ...
    Returns:
        Response: Rendered guest RSVP detail template or redirect
...
def photos():
    ...
    Returns:
        str: Rendered photos page template
    """
...
def hotel():
    """
    Render the hotel information page.
    
    Returns:
        str: Rendered hotel page template
    """
...
def registry():
    """
    Render the gift registry page with available items.
    
    Returns:
        str: Rendered registry page template with items and remaining quantities
    """
    session = get_db()
...
def registry_custom():
    """
    Handle custom gift amount registration.
    
    Returns:
        Response: Rendered custom gift page or redirect to payment
    """
    if request.method == 'POST':
        session = get_db()
...
def promise(item_id: int):
    """
    Handle gift promise creation.
    
    Args:
        item_id: ID of the gift item
        
    Returns:
        Response: Rendered promise page or redirect to registry
    """
    session = get_db()
...
def pay(item_id: int):
    """
    Handle payment for a given gift item.

    Yes, things are getting ugly here... The wedding date was closing in.
...
def post_pay():
    ...
    This route is called by Stripe after a payment is confirmed. It records the
    gift to the database.

    Returns:
        Response: Rendered post-pay page
    """
    stripe_key = current_app.config.get('STRIPE_API_KEY')
